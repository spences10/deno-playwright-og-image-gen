...
export class template_renderer {
  ...
  private get_template(template_name: string): string {
    ...
    const template_path = path.join(process.cwd(), 'src', 'templates', `${template_name}.html`);
  ...
  public render_template(template_name: string, data: Omit<template_data, 'background_colour' | 'text_colour' | 'accent_colour'>): string {
    const template = this.get_template(template_name);
    
    let rendered = template;
    
    // Replace template placeholders with sanitized data
    rendered = rendered.replace(/\{\{TITLE\}\}/g, this.sanitise_text(data.title));
    rendered = rendered.replace(/\{\{AUTHOR\}\}/g, this.sanitise_text(data.author));
    rendered = rendered.replace(/\{\{WEBSITE\}\}/g, this.sanitise_text(data.website));
